...
DEFAULT_STAR_RULE_ROWS = [
    {"scene": "Attendance", "behavior": "Participate in class and interact with the teacher."},
    {"scene": "Classwork", "behavior": "Complete in-class creations and after-class assignments."},
    {"scene": "Exam", "behavior": "Reach the required exam standard."},
    {"scene": "Check-in", "behavior": "Issue rewards automatically after check-in rules are configured."},
    {"scene": "OJ Zone", "behavior": "Submit correct problem solutions."},
    {"scene": "Report Share", "behavior": "Share study reports to receive likes."},
]
...
def _resolve_student_end_date(request: Request, payload: Any) -> str:
...
def _student_profile_context(store: MirrorStore) -> dict[str, Any]:
...
def _load_cached_student_work_rows(store: MirrorStore) -> list[dict[str, Any]]:
    context = _student_profile_context(store)
...
def _build_student_work_dataset(store: MirrorStore, request: Request) -> dict[str, Any]:
    ...
    captured_rows = _load_cached_student_work_rows(store)
    if captured_rows:
        rows = captured_rows
    else:
...
def _build_student_index_info_content(store: MirrorStore) -> dict[str, Any]:
    ...
    if "workNum" not in content:
        content["workNum"] = len(_load_cached_student_work_rows(store))
...
def _build_student_homepage_user_info(store: MirrorStore) -> dict[str, Any]:
    context = _student_profile_context(store)
...
def _build_homepage_content(store: MirrorStore, request: Request) -> dict[str, Any]:
    ...
    if request_profile == "student":
        ...
        user_info = _build_student_homepage_user_info(store)
    else:
...
def _student_state_with_route_context(
    ...
    *,
    ...
    student_user_info = _build_student_homepage_user_info(store)
    user_state = normalized_state.get("user")
...
def _build_student_auth_bootstrap(
    ...
    *,
    ...
    normalized_state = _student_state_with_route_context(store, profile_name, route_key=route_key)
    if not isinstance(normalized_state, dict):
        return None
...
def _build_route_bootstrap(
    ...
    if profile_role not in {"teacher", "student"}:
        return None
...
def _extract_student_ids(payload: Any, request: Request) -> list[int]:
    ...
    if isinstance(payload, dict):
        for key in ("stuIds", "ids", "studentIds", "chooseStuIds", "userIds"):
            if key in payload:
...
def _all_candidate_student_rows(store: MirrorStore) -> list[dict[str, Any]]:
    ...
    for row in _build_select_study_rows(store):
        ...
        rows_by_id[student_id] = {
            ...
            "xmGoodsList": [{"id": goods_id, "title": "Default Goods"}],
            "xm_goods_id": goods_id,
            "ClassGoodsInfo": {"id": goods_id, "student_user_id": student_id},
        }
    return sorted(
        rows_by_id.values(),
        key=lambda row: (
...
def _build_local_xm_goods_rows(student_row: dict[str, Any]) -> list[dict[str, Any]]:
...
def _build_local_xm_account_row(store: MirrorStore, student_row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "xmGoodsList": _build_local_xm_goods_rows(student_row),
    }
...
def _build_student_candidate_rows_for_class(
    ...
    *,
    ...
    if normalized_class_id is not None:
        ...
        for row in payload.get("studentList") if isinstance(payload, dict) else []:
            if not isinstance(row, dict):
                continue
...
def _build_student_candidate_rows_for_teaching_plan(
    ...
    *,
    ...
    if class_id is not None:
        ...
        for row in class_payload.get("studentList") if isinstance(class_payload, dict) else []:
            if not isinstance(row, dict):
                continue
            student_id = _coerce_int(row.get("student_user_id")) or _coerce_int(((row.get("studentInfo") or {}).get("id")))
            if student_id is not None:
...
def _default_header_rows(table_type: str) -> list[dict[str, Any]]:
    if table_type == "TCH_XMORDERINFO":
        return [
            {"code": "3", "selected": 1, "headDesc": "璁㈠崟绫诲瀷", "prop": "orderType"},
            {"code": "4", "selected": 1, "headDesc": "璐拱椤圭洰", "prop": "xmOrderGoodsStr"},
            {"code": "5", "selected": 1, "headDesc": "搴旀敹/搴旈€€", "prop": "amountOfMoney"},
            {"code": "6", "selected": 1, "headDesc": "瀹炴敹/瀹為€€", "prop": "amountOfMoney1"},
            {"code": "7", "selected": 1, "headDesc": "娆犺垂閲戦(鍏?", "prop": "unpaid_amount"},
            {"code": "8", "selected": 1, "headDesc": "Performance Owner", "prop": "belong_user_str"},
            {"code": "9", "selected": 1, "headDesc": "鍒涘缓鏃堕棿", "prop": "created_time"},
            {"code": "10", "selected": 1, "headDesc": "Last Payment Time", "prop": "last_pay_time"},
            {"code": "11", "selected": 1, "headDesc": "缁忓姙鏃堕棿", "prop": "deal_date"},
            {"code": "12", "selected": 1, "headDesc": "Order Status", "prop": "state"},
        ]
    return []
...
def _build_board_main_data_content(store: MirrorStore, request: Request) -> dict[str, Any]:
    ...
    students = store.list_local_students()
    plans = store.list_teaching_plans()
    today_formal_num = sum(1 for student in students if str(student.get("created_time") or "").startswith(today))
    today_lesson_plans = [
        plan
        for plan in plans
        if str(plan.get("class_date") or plan.get("start_class_date") or "").startswith(today)
    ]
    consume_hour = round(sum(_teaching_plan_cost_lesson_hour(plan) for plan in plans), 2)
    today_consume_hour = round(sum(_teaching_plan_cost_lesson_hour(plan) for plan in today_lesson_plans), 2)

    return {
        "intendNum": 0,
        "todayIntendNum": 0,
        "todayComeNum": 0,
        "formalNum": len(students),
        "tryNum": 0,
        "todayFormalNum": today_formal_num,
        "todayTryNum": 0,
        "lessonRecordNum": len(plans),
        ...
        "income": 0,
        "totalIncome": 0,
        "totalLessonHour": consume_hour,
        "todayIncome": 0,
        "todayLessonHour": today_consume_hour,
    }
...
def _normalize_student_fresh_data_content(store: MirrorStore, content: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if not isinstance(user_info, dict):
        user_info = {}
    normalized["userInfo"] = _merge_dict_defaults(user_info, _build_student_homepage_user_info(store))
...
def _build_local_api_fallback(store: MirrorStore, request: Request, request_body: bytes) -> dict[str, Any] | None:
    ...
    if path == "/api/admin/fresh/auth/user/data":
        ...
        payload = {
            ...
            "content": {
                ...
                or teacher_profile.get("username")
                or "",
                "authUserPermission": _teacher_admin_permissions(store, teacher_profile_name),
            },
            "error": {"message": "", "code": ""},
        }
        return {
    ...
    if path == "/api/tch/get/teaching/plan/list":
        rows = _build_teacher_teaching_plan_rows(store, request)
        class_id = _extract_class_id_from_request(request)
        if class_id is not None:
            rows = [
    ...
    if path == "/api/change/stu/class":
        ...
        source_class_id = _parse_int_like(
            _request_payload_value(request, submitted, "oldClassId", "sourceClassId", "fromClassId", "currentClassId")
        )
        target_class_id = _parse_int_like(
            _request_payload_value(request, submitted, "classId", "newClassId", "targetClassId", "curriculumClassId")
        )
    ...
    if path == "/api/stu/get/indexinfo/for/new":
        return _local_json_record(_success_payload(_build_student_index_info_content(store)))
    ...
    if path == "/api/xm/getStuInfoForFinacialPages":
        ...
        goods_rows = _build_local_xm_goods_rows(student_row)
        return _local_json_record(
            _success_payload(
                {
                    "stuBuyAmount": 0,
    ...
    if path == "/java-api/school/stu/setEndDate":
        submitted = _load_request_payload(request_body)
        student_ids = _extract_student_ids(submitted, request)
        end_date = _resolve_student_end_date(request, submitted)
    ...
    if path == "/java-api/school/currCls/delete":
        submitted = _load_request_payload(request_body)
        class_ids: list[int] = []
        if isinstance(submitted, dict):
...
def create_app(root: Path, *, allow_live_proxy: bool = True) -> FastAPI:
    ...
    async def replay_api(full_path: str, request: Request) -> Response:
        if request.url.path == "/java-api/auth/sch/freshAuthData":
        ...
        skip_cross_profile_lookup = (
            (profile_role == "teacher" and request.url.path in LOCAL_TEACHER_FALLBACK_PATHS)
        ...
        teacher_fallback_path = profile_role == "teacher" and request.url.path in LOCAL_TEACHER_FALLBACK_PATHS
        student_fallback_path = profile_role == "student" and request.url.path in LOCAL_STUDENT_FALLBACK_PATHS
        prefer_local_fallback = (
            (profile_role == "teacher" and request.url.path in LOCAL_TEACHER_PREFER_LOCAL_FALLBACK_PATHS)
    ...
    def frontend_asset(requested_path: str, request: Request) -> Response:
        ...
        return finalize_frontend_response(_static_response_or_404(shell_path))
...
def _local_login_response(store: MirrorStore, payload: dict[str, Any], *, expected_login_path: str) -> Response:
    ...
    if (
        ...
        or submitted_password_hash != profile_password_hash
    ):
        return JSONResponse(
            {
                "success": False,
...
def _local_fresh_auth_response(store: MirrorStore, request: Request) -> Response:
    ...
    message = None
    if profile and isinstance(profile["login_content"], dict):
        message = profile["login_content"].get("authTree")
    return JSONResponse(
        {
            "success": True,
            "content": {"flag": True, "message": message},
            "error": {"message": "", "code": ""},
        },
        status_code=200,
    )
